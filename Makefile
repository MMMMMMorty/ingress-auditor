...
test: manifests generate fmt vet setup-envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell "$(ENVTEST)" use $(ENVTEST_K8S_VERSION) --bin-dir "$(LOCALBIN)" -p path)" go test $$(go list ./... | grep -v /e2e) -coverprofile cover.out -timeout 15m
...
.PHONY: test-e2e
test-e2e: setup-test-e2e install-ingress-nginx manifests generate fmt vet ## Run the e2e tests using Minikube, increase the timmeout to 30 mins
	MINIKUBE=$(MINIKUBE) MINIKUBE_PROFILE=$(MINIKUBE_PROFILE) \
		go test -tags=e2e ./test/e2e/ -v -ginkgo.v -timeout 15m
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name ingress-auditor-builder
	$(CONTAINER_TOOL) buildx use ingress-auditor-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm ingress-auditor-builder
	rm Dockerfile.cross
